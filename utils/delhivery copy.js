...
router.put("/order/accept/:id", async (req, res) => {
  try {
    ...
    // Verify user authorization if needed
    const userId = req.header('x-user-id');
    const userPhone = req.header('x-phone');
    ...
    try {
      ...
      // Get product weight
      let weight = 500; // Default weight in grams if calculation fails
      try {
        weight = countWeight(order.product) || 500;
      } catch (weightErr) {
        console.log("Error calculating weight:", weightErr);
      }
      ...
      if (response.status === true) {
        ...
        await order.update({
          status: "PP", // Processing for Pickup
          trackingID: response.data.awb_number
        });
        
        return res.status(200).json({ 
          status: true, 
          message: "Order accepted and shipment created", 
          order: {
            id: order.id,
            status: order.status,
        ...
        // Fall back to just accepting the order if shipment creation fails
        ...
        return res.status(200).json({
          ...
          shippingError: "Could not create shipment with courier service"
        });
      }
    } catch (shippingError) {
