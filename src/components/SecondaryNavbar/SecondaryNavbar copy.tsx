...
export default function SecondaryNavbar() {
    ...
    if (categoryData?.categories?.length > 0) {
      categoryData.categories.forEach((category: Category) => {
        if (!categorySubcategories[category.id]) {
          fetch(`http://localhost:3002/subcategories/${category.id}`)
    ...
    if (hoveredCategory && categorySubcategories[hoveredCategory]) {
      categorySubcategories[hoveredCategory].forEach((subcategory) => {
        if (!subcategorySubcategories2[subcategory.id]) {
          fetch(`http://localhost:3002/admin/subcategories2/${subcategory.id}`)
            .then((response) => response.json())
            .then((data) => {
            ...
            > 
              ...
              <p className="cursor-pointer hover:bg-gray-100 px-1 py-1 text-[14px] not-italic font-normal leading-6 text-gray-600 hover:text-[#E70F0F]">
                Shop By Category
                            ...
                            {showCategories && (
                <div className="absolute top-full left-0 bg-white z-50 border shadow-md rounded-lg flex">
                  {/* Section 1: Main Categories Part 1 */}
                  <div className="w-[250px] bg-gray-50 rounded-l-lg">
                    <div className="py-2">
                      {categoryData?.categories?.sort((a: Category, b: Category) => a.id - b.id).map((category: Category) => (
                        <div
                          ...
                          className={`cursor-pointer px-4 py-3 text-[14px] not-italic font-normal leading-6 transition-colors ${
                  ...
                  {hoveredCategory && (
                    <div className="w-[750px] bg-white border-l border-gray-200">
                      <div className="py-4">
                        {categorySubcategories[hoveredCategory] ? (
                          <>
                            {/* Subcategories Grid - 3 Columns per Row */}
                            <div className="px-6">
                              ...
                              {categorySubcategories[hoveredCategory].map((subcategory) => {
                                ...
                                return (
                                  <div key={subcategory.id} className="space-y-3">
                                    ...
                                    >
                                      <h3 className="text-[14px] font-bold text-[#1C1C2F] hover:text-[#FF0012] transition-colors">
                                        {subcategory.name}
                                      </h3>
                                    </div>
                                    ...
                                    <div className="space-y-0.5">
                                      {subcategorySubcategories2[subcategory.id] && subcategorySubcategories2[subcategory.id].length > 0 && (
                                        subcategorySubcategories2[subcategory.id].map((subcategory2) => (
                                          <div
                                            key={subcategory2.id}
                                            ...
                                            className="cursor-pointer hover:text-[#FF0012] px-2 py-1 text-[13px] not-italic font-normal leading-5 text-gray-700 transition-colors"
                                          >
                                            {subcategory2.name}
                                          </div>
                                        ))
                          ...
                          <div className="px-4 py-2 text-[14px] text-gray-400">
  ...
  <Link key={offer.id} href={`/offers/${offer?.id}`}>
    <div className="px-2 cursor-pointer w-[100px] aspect-square bg-white">
      ...
      />
      <p className="cursor-pointer hover:bg-gray-100 px-5 py-2 text-[14px] not-italic font-normal leading-6">
        {offer.name}
      </p>
    </div>
  </Link>
