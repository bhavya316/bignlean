...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      // Call the send-login-otp API
      const response = await fetch(`${API_CONFIG.BASE_URL}/send-login-otp`, {
        method: "POST",
      ...
      if (data.status) {
        toast.dismiss();
        toast.success("Enter the OTP you received via Call");
        
        // Store phone in sessionStorage for OTP verification component
        sessionStorage.setItem("tempUserPhone", phone);
        sessionStorage.setItem("isRegistration", "false"); // This is for login
        ...
        setErrorMessage(data.message || "User not registered");
        toast.dismiss();
...
const handleFacebookSignIn = async () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Display error message when user not found */}
        {errorMessage && (
